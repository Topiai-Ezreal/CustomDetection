...
from custom_hnm_data import CustomHNMDataset
from custom_batch_sampler import CustomBatchSampler_svm
from dataset_prepare.util import save_model, check_dir
...
def load_data(data_root_dir):
    ...
    for name in ['train', 'val']:
        ...
        data_set = CustomSVMDataset(data_dir, label='aeroplane', dataset=name, transform=transform)
        if name is 'train':
            """
        ...
        sampler = CustomBatchSampler_svm(data_set.get_positive_num(), data_set.get_negative_num(), 32, 96)
...
def train_model(data_loaders, model, criterion, optimizer, lr_scheduler, num_epochs=25, device=None):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            print('{} - positive_num: {} - negative_num: {} - dataset_prepare size: {}'.format(
...
if __name__ == '__main__':
    ...
    proposal_path = os.path.join(root_path, 'Proposals', 'aeroplane')
    data_loaders, data_sizes = load_data(proposal_path)
